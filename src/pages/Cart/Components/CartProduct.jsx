...
const CartProduct = ({ shoe }) => {
  ...
  const quantityHandler = async (type) => {
    try {
      ...
      const {
        status,
        data: { cart },
      } = await cartItemQuantity({ id, encodedToken: token, type });
  ...
  return (
    <>
      <div className="p-4 w-auto h-54 m-2 grid grid-cols-2 gap-3 border rounded-xl">
        <div>
          <img
            src={img}
            alt=""
            className="w-56 -mt-10 cursor-pointer object-contain drop-shadow-2xl transition duration-500 hover:scale-110 hover:drop-shadow-2xl"
          />
        </div>
        <div className="flex flex-col gap-3">
          ...
          <div className="flex gap-4">
            ...
            <button
              className="button-theme blur-effect bg-slate-800 text-slate-200 w-full"
